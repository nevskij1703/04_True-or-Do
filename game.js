...
window.Game = (function () {
  ...
  async function completeCurrent() {
    ...
    if (state.cardsThisSession >= TOTAL_SLOTS || allUsed()) {
      // Конец партии. Порядок:
      //   1) Rate Us (если ещё не оценили). Если "Может позже" — внутри
      //      него же показывается interstitial-реклама.
      //   2) Только потом — финальный экран "Вечер удался" с кнопками
      //      "Сыграть снова" / "На главный".
  ...
  /* ===== Rate Us =====
   * Показываем модалку после 12-го "Выполнено", СРАЗУ перед финальным
   * end-overlay ("Вечер удался"). Висит до тех пор, пока игрок не
   * нажмёт "Оценить" хоть раз — тогда сохраняем флаг и больше не
   * беспокоим никогда.
   *
   * Полная цепочка завершения партии:
   *   12-е "Выполнено" → completeCurrent()
   *     → maybeShowRateModal()
   *         "Оценить"      → setRateGiven(true), без рекламы
   *         "Может позже"  → закрыть → interstitial → дальше
   *     → showEndOverlay()
   *     → юзер кликает "Сыграть снова" / "На главный"
   *
   ...
   *   и показом карточки. Между этой рекламой и Rate Us стоят
   *   user-action (прочитать 12-ю карточку → "Выполнено"). Так что
   *   back-to-back ad→RateUs архитектурно невозможен.
   * - "Может позже" — "цена" пропуска оценки: ПОСЛЕ Rate Us показывается
   *   interstitial, и только потом завершается ожидание maybeShow*.
   * - "Оценить" — без рекламы (игрок и так уже сделал доброе дело).
   */
